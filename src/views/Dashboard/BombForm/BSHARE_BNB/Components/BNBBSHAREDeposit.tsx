...
const depositTokenName = 'BSHARE-BNB-LP';

const Stake: React.FC = () => {
  const closedForStaking = false;
  const bombFinance = useBombFinance();
  const [approveStatus, approve] = useApprove(bombFinance.BNB, address);
  console.log('Bank Address :', bombFinance.BNB);
  const { color: themeColor } = useContext(ThemeContext);
  const tokenBalance = useTokenBalance(bombFinance.BNB);
    ...
    <Card>
      <CardContent>
        <StyledCardContentInner>
          ...
          <Label text={'BSHARE-BNB-LP'} variant="yellow" />
          <StyledCardActions>
